...
function ProjectCard({ project }: { project: Project }) {
  ...
  const handleDownload = async (project) => {
    try {
      ...
      // 3. Set the filename
      const safeName = (project.name || project._id || 'project').replace(/[^a-z0-9]/gi, '_');
      ...
      console.error("Error occurred in downloading the project", error);
      ...
      <div className="relative z-10 p-6 flex flex-col h-full">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="relative shrink-0">
            <button onClick={toggleMenu} className="p-2 rounded-xl hover:bg-muted/80 text-muted-foreground hover:text-foreground transition-colors">
              <MoreVertical className="w-5 h-5" />
            </button>
            
            {/* 👇 UPDATED: Passed onDownload prop here */}
            {isMenuOpen && (
                <ProjectMenu 
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {models.length > 0 ? (
              <>{displayModels.map((model, i) => (<span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-mono font-medium bg-violet-500/10 text-violet-600 dark:text-violet-300 border border-violet-500/20"><Database className="w-3 h-3 opacity-50" />{model.name}</span>))}{remainingModels > 0 && <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-medium bg-muted text-muted-foreground border border-border">+{remainingModels} more</span>}</>
...
const ProjectMenu = ({ onClose, onDownload }: { onClose: () => void; onDownload: () => void; }) => (
    <>
      ...
      <div className="absolute right-0 top-full mt-2 w-48 rounded-xl bg-popover border border-border shadow-xl z-40 animate-in fade-in zoom-in-95 duration-200 p-1">
        ...
        <button 
          ...
          className="w-full text-left px-3 py-2 rounded-lg text-sm text-foreground hover:bg-muted flex items-center gap-2 transition-colors"
        >
