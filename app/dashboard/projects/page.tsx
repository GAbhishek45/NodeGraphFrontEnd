...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        // ⚠️ CHANGED: No manual headers. We trust the browser cookie.
        const res = await axios.get(`${baseUrl}/api/generate/history`, {
           withCredentials: true // <--- This sends the "Cookie" header automatically
        });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { 
          responseType: 'blob',
          // ⚠️ CHANGED: Use withCredentials to send the cookie automatically
    ...
    >
      ...
      {project.img_url && (
        <div className="absolute inset-0 z-0">
           <img src={project.img_url} alt="Diagram Preview" className="w-full h-full object-cover opacity-[0.08] dark:opacity-[0.15] grayscale group-hover:opacity-20 group-hover:grayscale-0 transition-all duration-700 transform group-hover:scale-105" />
      ...
      <div className="relative z-10 p-6 flex flex-col h-full">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start gap-4 overflow-hidden">
            <div className={`p-3 rounded-2xl border transition-colors shrink-0 ${project.img_url ? 'bg-white/10 border-white/20 text-foreground backdrop-blur-md' : 'bg-violet-50 dark:bg-violet-900/20 text-violet-600 border-violet-100 dark:border-violet-800/50'}`}>
              {project.img_url ? <Code2 className="w-5 h-5" /> : <FolderGit2 className="w-5 h-5" />}
            </div>
            <div className="min-w-0">
              <h3 className="text-lg font-bold text-foreground group-hover:text-violet-500 transition-colors truncate pr-2">{displayName}</h3>
              <p className="text-sm text-muted-foreground mt-1 font-medium flex items-center gap-1.5"><Clock className="w-3 h-3" />{displayTime}</p>
            </div>
          </div>
                ...
                {models.length > 0 ? (
                    <>
                        {displayModels.map((model, i) => (
                            <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-mono font-medium bg-violet-500/10 text-violet-600 dark:text-violet-300 border border-violet-500/20"><Database className="w-3 h-3 opacity-50" />{model.name}</span>
                        ))}
                        {remainingModels > 0 && <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-medium bg-muted text-muted-foreground border border-border">+{remainingModels} more</span>}
                    </>
                ) : <span className="text-xs text-muted-foreground italic flex items-center gap-1"><AlertCircle className="w-3 h-3" /> No models detected</span>}
            </div>
        </div>
        <div className="mt-auto pt-4 border-t border-border/50 flex items-center justify-between">
           <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-mono bg-muted/50 px-2 py-1 rounded-md"><GitBranch className="w-3 h-3" />{displayBranch}</div>
...
const ProjectMenu = ({ onClose, onDownload, isDownloading }: { onClose: () => void; onDownload: () => void; isDownloading: boolean; }) => (
    <>
      ...
      <div className="absolute right-0 top-full mt-2 w-48 rounded-xl bg-popover border border-border shadow-xl z-40 animate-in fade-in zoom-in-95 duration-200 p-1">
        ...
        <button onClick={(e) => { e.stopPropagation(); onDownload(); }} disabled={isDownloading} className="w-full text-left px-3 py-2 rounded-lg text-sm text-foreground hover:bg-muted flex items-center gap-2 transition-colors disabled:opacity-50">
          {isDownloading ? <Loader2 className="w-3.5 h-3.5 animate-spin text-violet-500" /> : <Download className="w-3.5 h-3.5" />} {isDownloading ? "Zipping..." : "Download Code"}
        </button>
